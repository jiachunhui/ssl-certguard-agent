---
title: 常见问题
description: CertGuard Agent 高频问题与解决方案汇总：安装、注册、运行、证书部署、更新、卸载与安全。
---
...
> **注意**：v1.0.4 及以上版本的 `install.sh` 在**不加** `--keep-identity` 时，默认会清理旧身份并强制重新注册。因此执行时必须显式带上 `--keep-identity` 参数。
 ...
 详细说明见 [Nginx 证书部署教程](/tutorial/nginx) 的"需要手动处理的场景"。
...
# 如果确实是被 SELinux 拦截，设置正确的上下文
 ...
 > 如果您对安全有任何疑虑，请通过 **[服务支持](/support)** 联系我们获取帮助。
